...
// simd_search.rs is a standalone file, not a library module.
// We include it directly so it has access to the bwspi crate.
#[path = "../src/simd_search.rs"]
mod simd_search;
...
fn gen_uniform_width(n: usize, seed: u64) -> Vec<u64> {
    let mut rng = StdRng::seed_from_u64(seed);
    (0..n).map(|_| rng.gen_range((1u64 << 31)..=(1u64 << 32) - 1)).collect()
}
    ...
    for dist in &dists {
        ...
        for &size in &sizes {
            ...
            group.bench_with_input(
                BenchmarkId::new("bwspi_simd", size), &target, |b, &t| {
            ...
            // ── BWSPI scalar ──
            group.bench_with_input(
                BenchmarkId::new("bwspi_scalar", size), &target, |b, &t| {
            ...
            group.bench_with_input(
                BenchmarkId::new("swisstable", size), &target, |b, &t| {
            ...
            let std_map: HashMap<u64, usize> =
                data.iter().enumerate().map(|(i, &v)| (v, i)).collect();
            group.bench_with_input(
                BenchmarkId::new("hashmap_std", size), &target, |b, &t| {
            ...
            let ahash_map: AHashMap<u64, usize> =
                data.iter().enumerate().map(|(i, &v)| (v, i)).collect();
            group.bench_with_input(
                BenchmarkId::new("ahash", size), &target, |b, &t| {
            ...
            let fx_map: FxHashMap<u64, usize> =
                data.iter().enumerate().map(|(i, &v)| (v, i)).collect();
            group.bench_with_input(
                BenchmarkId::new("fxhash", size), &target, |b, &t| {
            ...
            group.bench_with_input(
                BenchmarkId::new("linear_scan", size), &target, |b, &t| {
    ...
    for &size in &sizes {
        ...
        group.bench_with_input(BenchmarkId::new("bwspi", size), &data, |b, data| {
            b.iter(|| {
                ...
                for &v in data { bwspi.insert(black_box(v)); }
                black_box(&bwspi);
            });
        });
        ...
        group.bench_with_input(BenchmarkId::new("swisstable", size), &data, |b, data| {
            b.iter(|| {
                ...
                for (i, &v) in data.iter().enumerate() { m.insert(black_box(v), i); }
                black_box(&m);
            });
        });
