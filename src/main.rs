...
fn gen_high_entropy(n: usize) -> Vec<u64> {
    ...
    (0..n).map(|_| {
        ...
        if bits == 0 { 0 }
            ...
            let min = 1u64 << (bits - 1);
            let max = if bits < 64 { (1u64 << bits) - 1 } else { u64::MAX };
            rng.range(min, max)
        }
    }).collect()
}
...
fn gen_uniform_width(n: usize) -> Vec<u64> {
    let mut rng = Xorshift64::new(42);
    (0..n).map(|_| rng.range(1u64 << 31, (1u64 << 32) - 1)).collect()
}
...
fn gen_clustered(n: usize) -> Vec<u64> {
    let mut rng = Xorshift64::new(42);
    let clusters = [
        (100u64, 200), (10_000, 20_000), (1_000_000, 2_000_000),
...
fn run_comparison(name: &str, data: &[u64]) {
    ...
    let hm_insert = time_it(|| {
        for (i, &v) in data.iter().enumerate() { std_map.insert(v, i); }
    });
    ...
    let vec_insert = time_it(|| { for &v in data { vec_data.push(v); } });
    ...
    println!("  ├──────────────────┼──────────────┼──────────────┼──────────────┼───────────────┤");
    println!("  │  BWSPI           │  {:>10}  │  {:>10}  │  {:>10}  │  {:>11}  │",
        fmt_dur(bwspi_insert), fmt_dur(bwspi_insert / n as u32), fmt_bytes(bwspi_heap), bwspi_allocs);
    println!("  │  HashMap (std)   │  {:>10}  │  {:>10}  │  {:>10}  │  {:>11}  │",
        fmt_dur(hm_insert), fmt_dur(hm_insert / n as u32), fmt_bytes(hm_heap), hm_allocs);
    println!("  │  Vec (append)    │  {:>10}  │  {:>10}  │  {:>10}  │  {:>11}  │",
        fmt_dur(vec_insert), fmt_dur(vec_insert / n as u32), fmt_bytes(vec_heap), vec_allocs);
    println!("  └──────────────────┴──────────────┴──────────────┴──────────────┴───────────────┘");

    // ── Search targets ──
    let hit_targets: Vec<u64> = data.iter()
        .step_by((n / SEARCH_ITERS).max(1))
        .copied().take(SEARCH_ITERS).collect();
    let miss_target = u64::MAX - 13;
    let iters = hit_targets.len() as u32;

    // ── Cold / Hot search — HIT ──
    let bwspi_hit = cold_hot_search(|| {
        for &t in &hit_targets { std::hint::black_box(bwspi.contains(std::hint::black_box(t))); }
    });
    let hm_hit = cold_hot_search(|| {
        for &t in &hit_targets { std::hint::black_box(std_map.get(&std::hint::black_box(t))); }
    });
    let lin_hit = cold_hot_search(|| {
        for &t in &hit_targets { std::hint::black_box(vec_data.iter().any(|&v| v == std::hint::black_box(t))); }
    });

    // ── Cold / Hot search — MISS ──
    let bwspi_miss = cold_hot_search(|| {
        for _ in 0..SEARCH_ITERS { std::hint::black_box(bwspi.contains(std::hint::black_box(miss_target))); }
    });
    let hm_miss = cold_hot_search(|| {
        for _ in 0..SEARCH_ITERS { std::hint::black_box(std_map.get(&std::hint::black_box(miss_target))); }
    });
    let lin_miss = cold_hot_search(|| {
        for _ in 0..SEARCH_ITERS { std::hint::black_box(vec_data.iter().any(|&v| v == std::hint::black_box(miss_target))); }
    });
    ...
    println!("  ├──────────────────┼───────────────────────────────┼───────────────────────────────────┤");
    println!("  │  BWSPI           │  {:>10}     {:>10}      │  {:>10}      {:>10}          │",
        fmt_dur(bwspi_hit.cold / iters), fmt_dur(bwspi_hit.hot_avg / iters),
        fmt_dur(bwspi_miss.cold / iters), fmt_dur(bwspi_miss.hot_avg / iters));
    println!("  │  HashMap (std)   │  {:>10}     {:>10}      │  {:>10}      {:>10}          │",
        fmt_dur(hm_hit.cold / iters), fmt_dur(hm_hit.hot_avg / iters),
        fmt_dur(hm_miss.cold / iters), fmt_dur(hm_miss.hot_avg / iters));
    println!("  │  Linear Search   │  {:>10}     {:>10}      │  {:>10}      {:>10}          │",
        fmt_dur(lin_hit.cold / iters), fmt_dur(lin_hit.hot_avg / iters),
        fmt_dur(lin_miss.cold / iters), fmt_dur(lin_miss.hot_avg / iters));
    println!("  └──────────────────┴───────────────────────────────┴───────────────────────────────────┘");
    ...
    println!("  ├──────────────────┼──────────────┼──────────────┼─────────────────────────┤");
    println!("  │  BWSPI           │  {:>10}  │  {:>10}  │  {:>21.2}x  │",
        fmt_bytes(bwspi_heap), fmt_bytes(bwspi_self_report), bwspi_heap as f64 / raw as f64);
    println!("  │  HashMap (std)   │  {:>10}  │           —  │  {:>21.2}x  │",
        fmt_bytes(hm_heap), hm_heap as f64 / raw as f64);
    println!("  │  Vec (raw)       │  {:>10}  │           —  │  {:>21.2}x  │",
        fmt_bytes(vec_heap), vec_heap as f64 / raw as f64);
    println!("  └──────────────────┴──────────────┴──────────────┴─────────────────────────┘");

    // ── Bucket distribution ──
    let stats = bwspi.distribution_stats();
    println!("\n  ┌─────────────────────────────────────────────────────────────────────────┐");
    println!("  │  BUCKET INDEX  ({} active / {} slots)                                  │",
        bwspi.active_bucket_count(), bwspi.bucket_slots());
    println!("  ├──────────┬──────────┬───────────┬────────────────────────────────────────┤");
    ...
    for s in &stats {
        ...
        println!("  │  bw={:<3}  │  {:>6}  │  {:>5.1}%   │  {:<37}│", s.bit_width, s.count, s.percentage, bar);
    }
    println!("  └──────────┴──────────┴───────────┴────────────────────────────────────────┘");

    let max = bwspi.max_bucket_size();
    let reduction = if n > 0 { (1.0 - max as f64 / n as f64) * 100.0 } else { 0.0 };
    println!("  📊 Worst bucket: {} / {} ({:.1}% search reduction) | {} slots allocated",
        max, n, reduction, bwspi.bucket_slots());
}
...
fn print_cpu_analysis() {
    ...
    println!();
    println!("  ┌────────────────────────┬───────────────────────────────────────────────────────────┐");
    println!("  │  System                │  Instructions / Cycle Cost                                │");
    println!("  ├────────────────────────┼───────────────────────────────────────────────────────────┤");
    println!("  │  BWSPI                 │  1 instruction (LZCNT)  — single cycle, no pipeline stall│");
    println!("  │  HashMap (SipHash)     │  ~60-80 instructions (4 rounds of SipHash-1-3)           │");
    println!("  │  HashMap (aHash)       │  ~15-20 instructions (AES-NI hardware rounds)            │");
    println!("  │  HashMap (FxHash)      │  ~5-8 instructions (multiply + shift)                    │");
    println!("  │  Linear Search         │  0 instructions (no routing — but scans everything)      │");
    println!("  └────────────────────────┴───────────────────────────────────────────────────────────┘");
    println!();
    println!("  ┌────────────────────────┬───────────────────────────────────────────────────────────┐");
    println!("  │  System                │  Worst-Case Resize Behavior                              │");
    println!("  ├────────────────────────┼───────────────────────────────────────────────────────────┤");
    println!("  │  BWSPI                 │  Bucket Vec grows (simple memcpy, never rehashes)        │");
    println!("  │  HashMap (any)         │  O(n) — rehash every key + realloc table                 │");
    println!("  │  Vec (linear)          │  O(n) — realloc copies entire data array                 │");
    println!("  └────────────────────────┴───────────────────────────────────────────────────────────┘");
}
...
fn print_tradeoffs() {
    ...
    println!();
    println!("  ┌──────────────────────────┬────────────┬────────────┬────────────┬──────────────┐");
    println!("  │  Property                │  BWSPI     │  HashMap   │  SwissTab  │  Linear Scan │");
    println!("  ├──────────────────────────┼────────────┼────────────┼────────────┼──────────────┤");
    println!("  │  Routing cost            │  1 cycle   │  60-80 cyc │  15-20 cyc │  0           │");
    println!("  │  Index size              │  Dynamic   │  Dynamic   │  Dynamic   │  None        │");
    println!("  │  Memory overhead         │  ~2x       │  ~3-4x     │  ~2-3x     │  1.0x        │");
    println!("  │  Worst-case insert       │  O(1)*     │  O(n)      │  O(n)      │  O(1)*       │");
    println!("  │  Search (best)           │  O(1)      │  O(1)      │  O(1)      │  O(n)        │");
    println!("  │  Search (worst)          │  O(n)      │  O(n)      │  O(n)      │  O(n)        │");
    println!("  │  Rehash latency spikes   │  NEVER     │  YES       │  YES       │  NEVER       │");
    println!("  │  Duplicate support       │  Native    │  Overwrites│  Overwrites│  Native      │");
    println!("  │  Cache-line friendly     │  YES       │  Moderate  │  YES       │  YES         │");
    println!("  └──────────────────────────┴────────────┴────────────┴────────────┴──────────────┘");
...
fn main() {
    println!();
    println!("╔════════════════════════════════════════════════════════════════════════════════════╗");
    println!("║  BWSPI — Bit-Width Sparse Pointer Index — Production Comparison Suite            ║");
    println!("║  Measuring: Speed · Heap · Allocations · Cold/Hot Cache · Distribution           ║");
    println!("╚════════════════════════════════════════════════════════════════════════════════════╝");

    // Verify bit_width
    println!("\n  🔬 bit_width verification:");
    for &(val, expected) in &[(0u64,0),(1,1),(2,2),(3,2),(255,8),(256,9),(u64::MAX,64)] {
        let ok = if bit_width(val) == expected { "✓" } else { "✗" };
